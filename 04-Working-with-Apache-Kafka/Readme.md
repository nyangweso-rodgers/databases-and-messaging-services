...
- **Messages** that have the same _key_ will always be sent to the same **partition**, and in the same order. The **key** is run through a hashing function which turns it into an integer. This output is then used to select a **partition**.
